...
describe('dataset helpers', () => {
  ...
  it('creates a valid limited ZIP with SQLite, CSV and matched Google time', async () => {
    const runtime = resolve('public/runtime/zeroperl.wasm');
    ...
    const result = await createDataset(files, 'limited', 'google', () => undefined, async () => new Response(await readFile(runtime)), () => sqlite);
  ...
  it('retains sidecar text in full mode', async () => {
    ...
    const result = await createDataset(files, 'full', 'google', () => undefined, async () => new Response(await readFile(runtime)), () => sqlite);
    const archive = unzipSync(result.archive);
